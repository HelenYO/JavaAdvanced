...
public class Implementor implements JarImpler {
    /**
     * Path separator specified to current System
     */
    private static final Character PATH_SEPARATOR = File.separatorChar;

    /**
     * Line separator specified to current System
     */
    ...
    public void implementJar(Class<?> token, Path jarFile) throws ImplerException {
        ...
        try {
            ...
            throw new ImplerException("Error during creating temp directory", e);
        }
        String cp = "";
        try {
    ...
    private class MethodWrapper {
        ...
        @Override
        public int hashCode() {
            return 57 * 57 * Arrays.hashCode(method.getParameterTypes())
                    + 57 * method.getName().hashCode()
    ...
    private String getBody(Executable exec) {
        String temp = getReturn(exec);
        String tmp = "";
        if (exec instanceof Constructor) {
            tmp =  getTabs(2) + "super(" + getArguments(exec, false) + ");" + END_LINE;
        }
        tmp = tmp + getTabs(2) + "return";
        if (temp.length() > 0) {
            tmp = tmp + " " + temp + ";";
        } else {
            tmp = tmp + ";";
